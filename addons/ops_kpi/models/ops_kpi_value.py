...
class OpsKpiValue(models.Model):
    ...
    def compute_kpi_value(self, kpi, period=None, branch_id=None, user_id=None):
        ...
        try:
            ...
            elif kpi.calculation_type == 'custom':
                if kpi.custom_formula:
                    try:
                        safe_eval_context = kpi._get_safe_eval_context()
                        result = safe_eval(kpi.custom_formula, safe_eval_context)
                        value = float(result) if result is not None else 0.0
                    except Exception as e:
                        _logger.warning("Custom KPI formula error for %s: %s", kpi.code, e)
                        value = 0.0
                else:
                    value = 0.0
            else:
