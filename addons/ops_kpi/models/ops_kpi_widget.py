...
class OpsKpiWidget(models.Model):
    """
    KPI Widget - Individual component on a KPI board.
    Can display KPIs, charts, lists, etc.
    """
    _name = 'ops.kpi.widget'
    _description = 'KPI Widget'
    _order = 'sequence, id'

    name = fields.Char(string='Widget Name', required=True)
    board_id = fields.Many2one('ops.kpi.board', string='KPI Board',
    ...
    def _get_list_data(self):
        ...
        model_name = self.list_model

        # Validate user has read access to the target model
        try:
