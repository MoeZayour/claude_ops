...
class OpsAssetDisposalWizard(models.TransientModel):
    ...
    def _onchange_asset_id(self):
        ...
        from its category, if they are set.
        """
    ...
    def _create_disposal_move(self):
        """
        Creates and returns the journal entry for the asset disposal.
        This method contains the core accounting logic for disposal.
        """
        self.ensure_one()
        asset = self.asset_id
        
        if not self.loss_account_id or not self.gain_account_id:
            raise UserError(_("Please configure the Gain and Loss accounts on the asset category or enter them manually."))

        move_lines = []
        
        # 1. Debit: Accumulated Depreciation
        # Credit the asset's accumulated depreciation to zero it out.
        ...
        # Credit the asset's original value from the asset account.
        move_lines.append((0, 0, {
        ...
        # For now, we assume disposal at zero value.
        # A more advanced wizard would have a "Sale Price" field.
        disposal_proceeds = 0 
        ...
        # Create the gain/loss line if there is a non-zero book value
        if book_value != 0:
            move_lines.append((0, 0, {
    ...
    def action_dispose_asset(self):
        """
        Main action called by the button. It creates the journal entry,
        posts it, and updates the asset's state.
        """
        self.ensure_one()
        ...
        # Update the asset state to 'disposed'
        self.asset_id.write({
            'state': 'disposed',
        ...
        # Return an action to view the created journal entry
        return {
