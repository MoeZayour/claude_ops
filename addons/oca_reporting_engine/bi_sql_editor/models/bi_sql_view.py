...
class BiSQLView(models.Model):
            ...
            if rec.view_order:
                for vtype in rec.view_order.split(","):
                    if vtype not in ("graph", "pivot", "list", "form"):
                        raise UserError(
                            _("Only graph, pivot, list or form views are supported")
                        )
    ...
    def button_reset_to_model_valid(self):
        ...
        views.mapped("list_view_id").unlink()
    ...
    def button_create_ui(self):
        ...
        self.list_view_id = self.env["ir.ui.view"].create(self._prepare_list_view()).id
        self.graph_view_id = (
    ...
    def _prepare_list_view(self):
        ...
        return {
            ...
            """</list>""".format(
                "".join([x._prepare_list_field() for x in self.bi_sql_view_field_ids])
            ),
        }
