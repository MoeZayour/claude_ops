...
class ReportXmlAbstract(models.AbstractModel):
    ...
    def generate_report(self, ir_report, docids, data=None):
        ...
        content = elist.tostring(
            elist.fromstring(result),
    ...
    def validate_report(self, xsd_schema_doc, content):
        ...
        ("XSD Validation Schema") of `ir.actions.report` via `elist` lib.
        ...
        if xsd_schema_doc:
            ...
            parsed_xsd_schema = elist.XML(decoded_xsd_schema_doc)
            xsd_schema = elist.XMLSchema(parsed_xsd_schema)
            parser = elist.XMLParser(schema=xsd_schema)

            try:
                # check content
                elist.fromstring(content, parser)
            except elist.XMLSyntaxError as error:
